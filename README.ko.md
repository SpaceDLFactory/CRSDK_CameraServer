...
- **라이브뷰** — MJPEG + 포커스 피킹, 3분할 그리드, 수동 회전
- **노출·색** — ISO·셔터·조리개·EV·화이트밸런스(+켈빈 슬라이더)·측광·드라이브·파일포맷·JPEG품질·Picture Profile
- **포커스** — MF Near/Far 슬라이더, 라이브뷰 클릭 AF 포인트(Y축 보정), AF 박스 크기 S/M/L, 반셔터(S1) + 합초 표시
