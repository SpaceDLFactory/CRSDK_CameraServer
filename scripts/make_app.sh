...
# build.rs가 cargo run용으로 박은 절대 dev rpath 제거 — 공개 바이너리에 홈 경로 노출 방지.
# (번들 dylib은 위 @executable_path rpath로 로드되므로 제거해도 동작 동일.)
install_name_tool -delete_rpath "$(pwd)/$SDK_LIB" "$APP/Contents/MacOS/$BIN" 2>/dev/null || true
