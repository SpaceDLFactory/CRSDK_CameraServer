...
# 전제: ./scripts/make_app.sh 로 dist/A7C Tether.app 가 먼저 만들어져 있어야 함.
# 사용: ./scripts/make_dmg.sh
# 결과: dist/A7C-Tether-v<버전>.dmg
set -euo pipefail
cd "$(dirname "$0")/.."

VERSION="$(grep '^version' crsdk_server/Cargo.toml | head -1 | cut -d'"' -f2)"
APP="dist/A7C Tether.app"
DMG="dist/A7C-Tether-v${VERSION}.dmg"
...
1) Drag "A7C Tether.app" onto the Applications folder.
2) First launch: right-click the app -> Open -> Open.
   (or run in Terminal:
      xattr -dr com.apple.quarantine "/Applications/A7C Tether.app" )
...
echo "▶ DMG 생성"
hdiutil create -volname "A7C Tether" -srcfolder "$STAGE" -ov -format UDZO "$DMG" >/dev/null
rm -rf "$STAGE"
